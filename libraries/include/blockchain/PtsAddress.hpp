...
{
   template<>
   struct hash<goopal::blockchain::PtsAddress> 
   {
       public:
         size_t operator()(const goopal::blockchain::PtsAddress &a) const 
         {
            size_t s;
...
FC_REFLECT( goopal::blockchain::PtsAddress, (addr) )

namespace fc 
{ 
   void to_variant( const goopal::blockchain::PtsAddress& var,  fc::variant& vo );
   void from_variant( const fc::variant& var,  goopal::blockchain::PtsAddress& vo );
}
