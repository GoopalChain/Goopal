...
namespace goopal { namespace blockchain {
   ...
   {
      WithdrawCondition():asset_id(0),slate_id(0),type(withdraw_null_type){}

      template<typename WithdrawType>
      WithdrawCondition( const WithdrawType& t, AssetIdType asset_id_arg = 0, SlateIdType delegate_id_arg = 0 )
      {
   ...
   struct MemoData
   {
       ...
       */
      fc::array<char,GOP_BLOCKCHAIN_MAX_MEMO_SIZE>     message;
   ...
   struct ExtendedMemoData
   {
      ...
      fc::array<char,GOP_BLOCKCHAIN_MAX_MEMO_SIZE>      message;
      fc::enum_type<uint8_t,MemoFlagsEnum>            memo_flags;
      fc::array<char,GOP_BLOCKCHAIN_EXTENDED_MEMO_SIZE> extra_memo;
   };
...
namespace fc {
   void to_variant( const goopal::blockchain::WithdrawCondition& var,  variant& vo );
   void from_variant( const variant& var,  goopal::blockchain::WithdrawCondition& vo );
   void to_variant( const goopal::blockchain::MemoData& var,  variant& vo );
   void from_variant( const variant& var,  goopal::blockchain::MemoData& vo );
}

FC_REFLECT_ENUM( goopal::blockchain::WithdrawConditionTypes,
        (withdraw_null_type)
        (withdraw_signature_type)
        (withdraw_multisig_type)
        (withdraw_escrow_type)
        )
FC_REFLECT( goopal::blockchain::WithdrawCondition,
        (asset_id)
        (slate_id)
        (type)
        (data)
        )
FC_REFLECT( goopal::blockchain::TransferMemo,
        (one_time_key)
        (encrypted_memo_data)
        )
FC_REFLECT_ENUM( goopal::blockchain::MemoFlagsEnum,
        (from_memo)
        (to_memo)
        )
FC_REFLECT( goopal::blockchain::MemoData,
        (from)
        (from_signature)
        (message)
        (memo_flags)
...
FC_REFLECT( goopal::blockchain::ExtendedMemoData,
        (from)
        (from_signature)
        (message)
        (memo_flags)
...
FC_REFLECT_DERIVED( goopal::blockchain::MemoStatus,
        (goopal::blockchain::MemoData),
        (has_valid_signature)
        (owner_private_key)
        )
FC_REFLECT( goopal::blockchain::WithdrawWithSignature,
        (owner)
        (memo)
        )

FC_REFLECT( goopal::blockchain::WithdrawWithMultisig,
        (required)
        (owners)
        (memo)
        )
FC_REFLECT( goopal::blockchain::withdraw_with_escrow,
        (sender)
        (receiver)
        (escrow)
        (agreement_digest)
