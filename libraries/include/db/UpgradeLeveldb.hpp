...
/**
 * This code has no gop dependencies, and it
...
/*
    ...
    database versioning. Originally, a database stored values of entry0, and
    entry was typedef'd to be entry0. A new type entry1 was created to add
    "new_field" to entry type, and entry was typedef'd to entry1. The typedef
    ...
    FC_REFLECT( entry0, (points) )
    REGISTER_DB_OBJECT(entry,0) //This creates an upgrade function for entry0 databases

    struct entry1
    {
        entry1() : points(0), new_field("EMPTY") {}
        
        entry1(const entry0& r0) //convert from entry0 to entry1 for legacy files
          {
          key = r0.key;
          new_field = "EMPTY";
          }
    ...
    FC_REFLECT( entry1, (points)(new_field) )

    typedef entry1 entry; //current databases store entry1 objects

    @endcode
*/

namespace goopal { namespace db {
