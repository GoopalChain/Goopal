...
namespace goopal { namespace net {
    namespace detail { class ChainDownloaderImpl; }
    ...
    class ChainDownloader {
        std::unique_ptr<detail::ChainDownloaderImpl> my;
      ...
      public:
         ...
         */
        ChainDownloader(std::vector<fc::ip::endpoint> chain_servers = std::vector<fc::ip::endpoint>());
        virtual ~ChainDownloader();
         ...
         */
        fc::future<void> get_all_blocks(std::function<void (const blockchain::FullBlock&, uint32_t)> new_block_callback,
