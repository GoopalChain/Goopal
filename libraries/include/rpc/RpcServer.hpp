...
namespace goopal { namespace rpc {
  ...
  /**
  *  @class rpc_server
  *  @brief provides a json-rpc interface to the goopal client
  */
  class RpcServer
  {
     public:
       RpcServer(goopal::client::Client* client);
       virtual ~RpcServer();

       bool        configure_rpc( const goopal::client::RpcServerConfig& cfg );
       bool        configure_http(const goopal::client::RpcServerConfig& cfg);
       bool        configure_encrypted_rpc(const goopal::client::RpcServerConfig& cfg);
       ...
       const goopal::api::MethodData& get_method_data(const std::string& method_name);
       std::vector<goopal::api::MethodData> get_all_method_data() const;
     ...
     protected:
       friend class goopal::rpc::detail::RpcServerImpl;

       void validate_method_data(goopal::api::MethodData method);
       void register_method(goopal::api::MethodData method);
