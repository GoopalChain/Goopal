...
#include <net/ChainServerCommands.hpp>
...
namespace goopal { namespace net {
    ...
    namespace detail {
      class ChainDownloaderImpl {
        public:
          ChainDownloader* self;
          ...
          void get_all_blocks(std::function<void (const blockchain::FullBlock&, uint32_t)> new_block_callback,
    ...
    ChainDownloader::ChainDownloader(std::vector<fc::ip::endpoint> chain_servers)
      : my(new detail::ChainDownloaderImpl)
    {
        my->self = this;
    ...
    void ChainDownloader::add_chain_server(const fc::ip::endpoint& server)
    {
        if(std::find(my->_chain_servers.begin(), my->_chain_servers.end(), server) == my->_chain_servers.end())
            my->_chain_servers.push_back(server);
    }

    void ChainDownloader::add_chain_servers(const std::vector<fc::ip::endpoint>& servers)
    {
    ...
    fc::future<void> ChainDownloader::get_all_blocks(std::function<void(const blockchain::FullBlock&,
                                                                         uint32_t)>
                                                               new_block_callback,
                                                      uint32_t first_block_number)
    {
