...
namespace goopal { namespace net
  {
    Message PeerConnection::RealQueuedMessage::get_message(PeerConnectionDelegate*)
    {
    ...
    size_t PeerConnection::RealQueuedMessage::get_size_in_queue()
    {
      return message_to_send.data.size();
    }
    Message PeerConnection::VirtualQueuedMessage::get_message(PeerConnectionDelegate* node)
    {
      return node->get_message_for_item(item_to_send);
    }

    size_t PeerConnection::VirtualQueuedMessage::get_size_in_queue()
    {
      return sizeof(ItemId);
    }
    PeerConnection::PeerConnection(PeerConnectionDelegate* delegate) :
      _node(delegate),
      _message_connection(this),
      _total_queued_messages_size(0),
      direction(PeerConnectionDirection::unknown),
      is_firewalled(FirewalledState::unknown),
      our_state(OurConnectionState::disconnected),
      they_have_requested_close(false),
      their_state(TheirConnectionState::disconnected),
      we_have_requested_close(false),
      negotiation_status(ConnectionNegotiationStatus::disconnected),
    ...
    PeerConnectionPtr PeerConnection::make_shared(PeerConnectionDelegate* delegate)
    {
      ...
      // to cancel it.
      return PeerConnectionPtr(new PeerConnection(delegate));
    ...
    fc::tcp_socket& PeerConnection::get_socket()
    {
      VERIFY_CORRECT_THREAD();
      return _message_connection.get_socket();
    }

    void PeerConnection::accept_connection()
    {
      VERIFY_CORRECT_THREAD();
      ...
      try
      {
        assert( our_state == OurConnectionState::disconnected &&
                their_state == TheirConnectionState::disconnected );
        direction = PeerConnectionDirection::inbound;
        negotiation_status = ConnectionNegotiationStatus::accepting;
        _message_connection.accept();           // perform key exchange
        negotiation_status = ConnectionNegotiationStatus::accepted;
        ...
        their_state = TheirConnectionState::just_connected;
        our_state = OurConnectionState::just_connected;
    ...
    void PeerConnection::connect_to( const fc::ip::endpoint& remote_endpoint, fc::optional<fc::ip::endpoint> local_endpoint )
    {
      VERIFY_CORRECT_THREAD();
      try
      {
        ...
        negotiation_status = ConnectionNegotiationStatus::connecting;
        _message_connection.connect_to( remote_endpoint );
        negotiation_status = ConnectionNegotiationStatus::connected;
        their_state = TheirConnectionState::just_connected;
        our_state = OurConnectionState::just_connected;
    ...
    void PeerConnection::on_message( MessageOrientedConnection* originating_connection, const Message& received_message )
    {
      VERIFY_CORRECT_THREAD();
      _node->on_message( this, received_message );
    }

    void PeerConnection::on_connection_closed( MessageOrientedConnection* originating_connection )
    {
      VERIFY_CORRECT_THREAD();
      negotiation_status = ConnectionNegotiationStatus::closed;
      _node->on_connection_closed( this );
    }

    void PeerConnection::send_queued_messages_task()
    {
      VERIFY_CORRECT_THREAD();
#ifndef NDEBUG
      struct counter {
    ...
    void PeerConnection::send_queueable_message(std::unique_ptr<QueuedMessage>&& message_to_send)
    {
      VERIFY_CORRECT_THREAD();
      _total_queued_messages_size += message_to_send->get_size_in_queue();
      _queued_messages.emplace(std::move(message_to_send));
      if (_total_queued_messages_size > GOP_NET_MAXIMUM_QUEUED_MESSAGES_IN_BYTES)
      {
        elog("send queue exceeded maximum size of ${max} bytes (current size ${current} bytes)",
             ("max", GOP_NET_MAXIMUM_QUEUED_MESSAGES_IN_BYTES)("current", _total_queued_messages_size));
        try
        {
          close_connection();
        }
    ...
    void PeerConnection::send_message(const Message& message_to_send, size_t message_send_time_field_offset)
    {
      VERIFY_CORRECT_THREAD();
      dlog("peer_connection::send_message() enqueueing message of type ${type} for peer ${endpoint}",
           ("type", message_to_send.msg_type)("endpoint", get_remote_endpoint()));
      std::unique_ptr<QueuedMessage> message_to_enqueue(new RealQueuedMessage(message_to_send, message_send_time_field_offset));
      send_queueable_message(std::move(message_to_enqueue));
    }

    void PeerConnection::send_item(const ItemId& item_to_send)
    {
      VERIFY_CORRECT_THREAD();
      ...
      std::unique_ptr<QueuedMessage> message_to_enqueue(new VirtualQueuedMessage(item_to_send));
      send_queueable_message(std::move(message_to_enqueue));
    }
    ...
    {
      VERIFY_CORRECT_THREAD();
      negotiation_status = ConnectionNegotiationStatus::closing;
    ...
    void PeerConnection::destroy_connection()
    {
      VERIFY_CORRECT_THREAD();
      negotiation_status = ConnectionNegotiationStatus::closing;
      destroy();
    ...
    uint64_t PeerConnection::get_total_bytes_sent() const
    {
      VERIFY_CORRECT_THREAD();
      return _message_connection.get_total_bytes_sent();
    }

    uint64_t PeerConnection::get_total_bytes_received() const
    {
      VERIFY_CORRECT_THREAD();
      return _message_connection.get_total_bytes_received();
    }

    fc::time_point PeerConnection::get_last_message_sent_time() const
    {
      VERIFY_CORRECT_THREAD();
      return _message_connection.get_last_message_sent_time();
    }

    fc::time_point PeerConnection::get_last_message_received_time() const
    {
      VERIFY_CORRECT_THREAD();
      return _message_connection.get_last_message_received_time();
    }

    fc::optional<fc::ip::endpoint> PeerConnection::get_remote_endpoint()
    {
      VERIFY_CORRECT_THREAD();
      return _remote_endpoint;
    }
    fc::ip::endpoint PeerConnection::get_local_endpoint()
    {
      VERIFY_CORRECT_THREAD();
      return _message_connection.get_socket().local_endpoint();
    }

    void PeerConnection::set_remote_endpoint( fc::optional<fc::ip::endpoint> new_remote_endpoint )
    {
      VERIFY_CORRECT_THREAD();
      _remote_endpoint = new_remote_endpoint;
    }

    bool PeerConnection::busy()
    {
      VERIFY_CORRECT_THREAD();
      return !items_requested_from_peer.empty() || !sync_items_requested_from_peer.empty() || item_ids_requested_from_peer;
    }

    bool PeerConnection::idle()
    {
      VERIFY_CORRECT_THREAD();
      return !busy();
    }

    bool PeerConnection::is_transaction_fetching_inhibited() const
    {
      VERIFY_CORRECT_THREAD();
      return transaction_fetching_inhibited_until > fc::time_point::now();
    }

    fc::sha512 PeerConnection::get_shared_secret() const
    {
      VERIFY_CORRECT_THREAD();
      return _message_connection.get_shared_secret();
    }

    void PeerConnection::clear_old_inventory()
    {
      VERIFY_CORRECT_THREAD();
      fc::time_point_sec oldest_inventory_to_keep(fc::time_point::now() - fc::minutes(GOP_NET_MAX_INVENTORY_SIZE_IN_MINUTES));
    ...
    bool PeerConnection::is_inventory_advertised_to_us_list_full_for_transactions() const
    {
      VERIFY_CORRECT_THREAD();
      return inventory_peer_advertised_to_us.size() > GOP_NET_MAX_INVENTORY_SIZE_IN_MINUTES * GOP_BLOCKCHAIN_MAX_TRX_PER_SECOND * 60;
    }

    bool PeerConnection::is_inventory_advertised_to_us_list_full() const
    {
      VERIFY_CORRECT_THREAD();
      // allow the total inventory size to be the maximum number of transactions we'll store in the inventory (above)
      // plus the maximum number of blocks that would be generated in GOP_NET_MAX_INVENTORY_SIZE_IN_MINUTES (plus one,
      // to give us some wiggle room)
      return inventory_peer_advertised_to_us.size() >
        GOP_NET_MAX_INVENTORY_SIZE_IN_MINUTES * GOP_BLOCKCHAIN_MAX_TRX_PER_SECOND * 60 +
        (GOP_NET_MAX_INVENTORY_SIZE_IN_MINUTES + 1) * 60 / GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
    }

    bool PeerConnection::performing_firewall_check() const
    {
      return firewall_check_state && firewall_check_state->requesting_peer != NodeIdType();
    }

    fc::optional<fc::ip::endpoint> PeerConnection::get_endpoint_for_connecting() const
    {
      if (inbound_port)
