...
namespace goopal { namespace cli {
                                       ...
                                       ClientRawPtr client, std::ostream& out )const
  {
    try
    {
      ...
      else
      {
        ...
        const goopal::api::MethodData& method_data = client->get_rpc_server()->get_method_data( method_name);
        result_type = method_data.return_type;

        if( result_type == "asset")
        {
          out << string( result.as<goopal::blockchain::Asset>() ) << "\n";
        }
        else if(result_type == "address")
        {
          out << (string)result.as<goopal::blockchain::Address>() << "\n";
        }
  ...
  void PrintResult::f_wallet_transfer(std::ostream& out, const fc::variants& arguments, const fc::variant& result, ClientRawPtr client )
  {
    ...
    const auto& pretty = client->get_wallet()->to_pretty_trx(entry);
  ...
  {
    auto transactions = result.as<vector<SignedTransaction>>();
