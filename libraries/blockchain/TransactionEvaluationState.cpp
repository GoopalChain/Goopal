   ...
   { try {
      ...
      for( const auto& fee : balance )
      {
         ...
         if( fee.second > 0 ) // if a fee was paid...
         {
            ...
            asset_entry->collected_fees += fee.second;
            _current_state->store_asset_entry( *asset_entry );
         }
      }

	  /* if( _current_state->get_head_block_num() >= GOP_V0_6_4_FORK_BLOCK_NUM )
      {
          for( const auto& op : trx.operations )
          {
              if( operation_type_enum( op.type ) == cover_op_type )
              {
                  FC_ASSERT( balance[ 0 ] <= GOP_BLOCKCHAIN_PRECISION );
              }
          }
      }*/
   } FC_CAPTURE_AND_RETHROW() }

   void TransactionEvaluationState::evaluate( const SignedTransaction& trx_arg )
   { try {
      trx = trx_arg;
      try {
        ...
        if( (_current_state->now() + GOP_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC) < trx_arg.expiration )
