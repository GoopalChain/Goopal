...
namespace goopal { namespace blockchain {
   ...
   namespace detail
   {
      void ChainDatabaseImpl::revalidate_pending()
      {
            ...
            while( itr.valid() )
            {
                ...
                try
                {
                  TransactionEvaluationStatePtr eval_state = self->evaluate_transaction( trx, _relay_fee );
      ...
      { try {
          ...
          return !entry.valid() || entry->value.as_uint64() != GOP_BLOCKCHAIN_DATABASE_VERSION;
      } FC_CAPTURE_AND_RETHROW( (data_dir) ) }
      ...
      { try {
		  ...
		  _gop_input_balance_entry.open(data_dir / "index/_gop_input_balance_entry");
		  _gop_full_entry.open(data_dir/"index/_gop_full_entry");
      ...
      { try {
         ...
         // Check genesis state
         FC_ASSERT( config.delegates.size() >= GOP_BLOCKCHAIN_NUM_DELEGATES,
                    "genesis.json does not contain enough initial delegates!",
                    ("required",GOP_BLOCKCHAIN_NUM_DELEGATES)("provided",config.delegates.size()) );
         ...
         {
             //static const vector<string> gop_prefixes{ "GOP", "KEY", "DVS", "XTS" };
             try
             {
             ...
             {
                 //for( const string& prefix : gop_prefixes )
                 //{
                    // if( raw_address.find( prefix ) == 0 )
                   //  {
                       //  return address( GOP_ADDRESS_PREFIX + raw_address.substr( prefix.size() ) );
                    // }
                 //}
             }
         ...
         base_asset.symbol = GOP_BLOCKCHAIN_SYMBOL;
         base_asset.name = GOP_BLOCKCHAIN_NAME;
         base_asset.description = GOP_BLOCKCHAIN_DESCRIPTION;
         base_asset.public_data = variant("");
         base_asset.issuer_account_id = 0;
         base_asset.precision = GOP_BLOCKCHAIN_PRECISION;
         base_asset.registration_date = timestamp;
         base_asset.last_update = timestamp;
         base_asset.current_share_supply = total_base_supply;
         base_asset.maximum_share_supply = GOP_BLOCKCHAIN_MAX_SHARES;
         ...
         for( const GenesisAsset& asset : config.market_assets )
         {
             ...
             rec.maximum_share_supply = GOP_BLOCKCHAIN_MAX_SHARES;
      ...
      { try {
         // if( pending_state->get_head_block_num() < GOP_V0_6_0_FORK_BLOCK_NUM )
      ...
      { try {
          ...
          if( block_num > GOP_BLOCKCHAIN_MAX_UNDO_HISTORY )
          {
              const uint32_t old_block_num = block_num - GOP_BLOCKCHAIN_MAX_UNDO_HISTORY;
      ...
      { try {
          ...
          if( block_digest.timestamp.sec_since_epoch() % GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC != 0 )
          ...
          fc::time_point_sec now = goopal::blockchain::now();
          auto delta_seconds = (block_digest.timestamp - now).to_seconds();
          if( block_digest.timestamp >  (now + GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC*2) )
      ...
      /**
       *  A block should be produced every GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC. If we do not have a
      ...
      { try {
          ...
          if( head_block.block_num > 0 ) block_timestamp = head_block.timestamp + GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
          ...
          for( ; block_timestamp < block_header.timestamp;
                 block_timestamp += GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
          ...
          if( required_confirmations > GOP_BLOCKCHAIN_NUM_DELEGATES*3 )
             required_confirmations = 3*GOP_BLOCKCHAIN_NUM_DELEGATES;
      ...
      void ChainDatabaseImpl::update_active_delegate_list( const uint32_t block_num,
                                                             const PendingChainStatePtr& pending_state )const
      { try {//todo remove????
         // if( pending_state->get_head_block_num() < GOP_V0_7_0_FORK_BLOCK_NUM )
             // return update_active_delegate_list_v1( block_num, pending_state );

          if( block_num % GOP_BLOCKCHAIN_NUM_DELEGATES != 0 )
              return;
      ...
      { try {
         ...
         if( (now() - block_data.timestamp).to_seconds() < GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC )
      ...
      { try {
         ...
         goopal::blockchain::PendingChainStatePtr undo_state_ptr = std::make_shared<goopal::blockchain::PendingChainState>( undo_state );
   ...
   {
       return get_delegates_by_vote( 0, GOP_BLOCKCHAIN_NUM_DELEGATES );
   }
   ...
   { try {
      ...
      try
      {
          ...
          else
          {
              ...
              my->open_database( data_dir );
              store_property_entry( PropertyIdType::database_version, variant( GOP_BLOCKCHAIN_DATABASE_VERSION ) );
              ...
              const auto insert_block = [&]( const FullBlock& block )
              {
                  if( blocks_indexed % 200 == 0 )
                  {
                      ...
                      else
                      {
                          ...
                          progress = float( blocks_indexed * GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC ) / seconds;
                      }
              ...
              else
              {
                  ...
                  if( last_known_block_num > GOP_BLOCKCHAIN_MAX_UNDO_HISTORY )
                      my->_min_undo_block = last_known_block_num - GOP_BLOCKCHAIN_MAX_UNDO_HISTORY;
   ...
   { try {
	  ...
	  my->_gop_input_balance_entry.close();
	  my->_gop_full_entry.close();
   ...
   { try {
      ...
      auto delegate_pos = slot_number % GOP_BLOCKCHAIN_NUM_DELEGATES;
   ...
   { try {
      ...
      if( next_block_time <= now() ) next_block_time += GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
      auto last_block_time = next_block_time + (GOP_BLOCKCHAIN_NUM_DELEGATES * GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC);

      auto active_delegates = get_active_delegates();
      for( ; next_block_time < last_block_time; next_block_time += GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC )
      {
   ...
   { try {
      ...
      trx_eval_state->evaluate( trx );
      const ShareType fees = trx_eval_state->get_fees() + trx_eval_state->alt_fees_paid.amount;
   ...
   { try {
      ...
      if( head_block_num > GOP_BLOCKCHAIN_MAX_UNDO_HISTORY &&
          block_data.block_num <= (head_block_num - GOP_BLOCKCHAIN_MAX_UNDO_HISTORY) )
      {
        elog( "block ${new_block_hash} (number ${new_block_num}) is on a fork older than "
               "our undo history would allow us to switch to (current head block is number ${head_block_num}, undo history is ${undo_history})",
                           ...
                           ("head_block_num", head_block_num)("undo_history", GOP_BLOCKCHAIN_MAX_UNDO_HISTORY));
   ...
   */
   double ChainDatabase::get_average_delegate_participation()const
   {
	   try {
		   ...
		   const auto now = goopal::blockchain::now();
		   if (head_num < 1)
		   {
			   return 0;
		   }
		   else if (head_num <= GOP_BLOCKCHAIN_NUM_DELEGATES)
		   {
			   // what percent of the maximum total blocks that could have been produced
			   // have been produced.
			   const auto expected_blocks = (now - get_block_header(1).timestamp).to_seconds() / GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
		   ...
		   else
		   {
			   ...
			   const auto starting_time = get_block_header(head_num - GOP_BLOCKCHAIN_NUM_DELEGATES).timestamp;
			   const auto expected_production = (now - starting_time).to_seconds() / GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
			   return 100 * double(GOP_BLOCKCHAIN_NUM_DELEGATES) / expected_production;
		   }
	   } FC_RETHROW_EXCEPTIONS(warn, "")
   }
   ...
   TransactionEvaluationStatePtr ChainDatabase::store_pending_transaction( const SignedTransaction& trx, bool override_limits )
   { try {
      auto trx_id = trx.id();
      if (override_limits)
      ...
      if( !override_limits )
      {
         if( my->_pending_fee_index.size() > GOP_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE )
         {
             auto overage = my->_pending_fee_index.size() - GOP_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
             relay_fee = my->_relay_fee * overage * overage;
         }
      }

      TransactionEvaluationStatePtr eval_state = evaluate_transaction( trx, relay_fee );
      const ShareType fees = eval_state->get_fees() + eval_state->alt_fees_paid.amount;
   ...
   { try {
      ...
      //if( pending_state->get_head_block_num() >= GOP_V0_4_9_FORK_BLOCK_NUM )
      ...
      if( config.block_max_transaction_count > 0 && config.block_max_size > block_size )
      {
          ...
          for( const TransactionEvaluationStatePtr& item : pending_trx )
          {
              ...
              try
              {
                  ...
                  auto pending_trx_state = std::make_shared<PendingChainState>( pending_state );
                  {
     ...
     // if( new_block.block_num < GOP_V0_7_0_FORK_BLOCK_NUM )
   ...
   { try {
    ...
    string ChainDatabase::export_fork_graph( uint32_t start_block, uint32_t end_block, const fc::path& filename )const
    {
      ...
      for( auto block_itr = my->_block_id_to_full_block.begin(); block_itr.valid(); ++block_itr )
      {
        ...
        {
          unsigned rank = (unsigned)((block.timestamp - start_time).to_seconds() / GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
   ...
   { try {
            ...
            iter != my->_account_id_to_entry.unordered_end(); ++iter )
       {
           ...
           GenesisBalance balance;
           balance.raw_address = string( entry.active_address() );
   ...
   vector<GopTrxidBalance> ChainDatabase::fetch_gop_input_balance(const uint32_t & block_num)
   {
	   try{
		   vector < GopTrxidBalance > results;
		   for (auto iter = my->_gop_input_balance_entry.unordered_begin();
			   iter != my->_gop_input_balance_entry.unordered_end(); ++iter)
		   {
			   GopTrxidBalance gopTemp;
			   gopTemp.block_num = block_num+1;
			   set<GopTrxidBalance>::iterator block_iter = iter->second.lower_bound(gopTemp);
   ...
   void ChainDatabase::transaction_insert_to_gop_full_entry(const string & gop_account, const GopTrxidBalance & gop_balance_record)
   {
	   auto iter_entry = my->_gop_full_entry.unordered_find(gop_account);
	   GopBalanceEntry gopentrys;
	   if (iter_entry != my->_gop_full_entry.unordered_end())
	   {
		   gopentrys = iter_entry->second;
		   if (gopentrys.gop_trxid_sort.count(gop_balance_record.trx_id) != 0)
		   {
			   return;
		   }
	   }
	   gopentrys.gop_trxid_sort.insert(gop_balance_record.trx_id);
	   gopentrys.gop_block_sort.insert(std::make_pair(gop_balance_record.block_num, gop_balance_record));
	   my->_gop_full_entry.store(gop_account, gopentrys);
   }
   void ChainDatabase::transaction_erase_from_gop_full_entry(const string & gop_account, const GopTrxidBalance & gop_balance_record)
   {
	   auto iter_entry = my->_gop_full_entry.unordered_find(gop_account);
	   if (iter_entry != my->_gop_full_entry.unordered_end())
	   {
		   GopBalanceEntry gopentrys = iter_entry->second;
		   gopentrys.gop_trxid_sort.erase(gop_balance_record.trx_id);
		   auto iter = gopentrys.gop_block_sort.lower_bound(gop_balance_record.block_num);
		   for (; iter != gopentrys.gop_block_sort.end();)
		   {
			   if (iter->second.block_num != gop_balance_record.block_num)
			   {
				   break;
			   }
			   if (iter->second.trx_id == gop_balance_record.trx_id)
			   {
				   iter = gopentrys.gop_block_sort.erase(iter);
			   }
			   else
			   {
				   ++iter;
		   ...
		   if (!(gopentrys.gop_block_sort.empty()))
		   {
			   my->_gop_full_entry.store(gop_account, gopentrys);
		   }
		   else
		   {
			   my->_gop_full_entry.remove(gop_account);
		   }
	   }
   }
   void ChainDatabase::transaction_insert_to_gop_balance(const string & gop_account,const GopTrxidBalance & gop_balance_entry)
   {
	   auto iter_ids = my->_gop_input_balance_entry.unordered_find(gop_account);
	   set<GopTrxidBalance> ids;
	   if (iter_ids != my->_gop_input_balance_entry.unordered_end())
	   {
		   ids = iter_ids->second;
	   }
	   ids.insert(gop_balance_entry);
	   my->_gop_input_balance_entry.store(gop_account, ids);
   }
   void ChainDatabase::transaction_erase_from_gop_balance(const string & gop_account, const GopTrxidBalance & gop_balance_entry)
   {
	   auto iter_ids = my->_gop_input_balance_entry.unordered_find(gop_account);
	   if (iter_ids != my->_gop_input_balance_entry.unordered_end())
	   {
		   auto ids = iter_ids->second;
		   ids.erase(gop_balance_entry);
		   if (!(ids.empty()))
		   {
			   my->_gop_input_balance_entry.store(gop_account, ids);
		   }
		   ...
		   {
			   my->_gop_input_balance_entry.remove(gop_account);
		   }
	   }
   }
   ...
   { try {
	   ...
	   */
       ...
       ulog( "Dumped ${p}", ("p",next_path) );
	   next_path = dir / "gop_transaction_balance_db.json";
	   my->_gop_input_balance_entry.export_to_json(next_path);
