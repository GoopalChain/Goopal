...
namespace goopal { namespace blockchain {
  ...
  BalanceIdType DepositOperation::balance_id()const
   ...
   { try {
      ...
      // if( eval_state._current_state->get_head_block_num() >= GOP_V0_6_0_FORK_BLOCK_NUM )
       //{
   ...
   { try {
      ...
      FC_ASSERT( total_released <= escrow_balance_entry->balance );
   ...
   { try {
      ...
      FC_ASSERT( current_balance_entry->condition.asset_id == 0, "Only GOP balances can have restricted owners." );
      ...
      auto fee = GOP_BLOCKCHAIN_PRECISION / 2;
      ...
      else // NOT this->new_restricted_owner.valid() || (this->new_restricted_owner == new_restricted_owner)
      {
          ...
          FC_ASSERT( eval_state._current_state->now().sec_since_epoch() - last_update_secs
                     >= GOP_BLOCKCHAIN_VOTE_UPDATE_PERIOD_SEC,
