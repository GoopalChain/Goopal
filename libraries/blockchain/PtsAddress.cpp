...
namespace fc 
{ 
   void to_variant( const goopal::blockchain::PtsAddress& var,  variant& vo )
   {
        vo = std::string(var);
   }
   void from_variant( const variant& var,  goopal::blockchain::PtsAddress& vo )
   {
        vo = goopal::blockchain::PtsAddress( var.as_string() );
   }
}
