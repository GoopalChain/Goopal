...
namespace goopal { namespace blockchain {
  ...
  fc::ecc::public_key ExtendedPrivateKey::get_public_key()const
  {
   ...
   { try {
      ...
      FC_ASSERT( base58str.size() > prefix_len,"Address must be longer than GOP_ADDRESS_PREFIX");
      FC_ASSERT( base58str.substr( 0, prefix_len ) == prefix,"GOP_ADDRESS_PREFIX must be in front of the address" );
...
namespace fc
{
   void to_variant( const goopal::blockchain::ExtendedAddress& ext_addr,  fc::variant& vo )
   {
   ...
   {
      vo = goopal::blockchain::ExtendedAddress( var.as_string() );
   }
}
