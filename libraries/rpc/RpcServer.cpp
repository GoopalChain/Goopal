...
namespace goopal { namespace rpc {
  ...
  namespace detail
  {
    class RpcServerImpl : public goopal::rpc_stubs::CommonApiRpcServer
    {
       public:
         RpcServerConfig                                 _config;
         goopal::client::Client*                              _client;
         ...
         typedef std::map<std::string, goopal::api::MethodData> MethodMapType;
         ...
         RpcServerImpl(goopal::client::Client* client) :
           _client(client),
         ...
         virtual goopal::api::CommonApi* get_client() const override;
         ...
         virtual void store_method_metadata(const goopal::api::MethodData& method_metadata);

         std::string help(const std::string& command_name) const;

         std::string make_short_description(const goopal::api::MethodData& method_data, bool show_decription = true) const
         {
           std::string help_string;
           std::stringstream short_description;
           short_description << std::setw(100) << std::left;
           help_string = method_data.name + " ";
           for (const goopal::api::ParameterData& parameter : method_data.parameters)
           {
             if (parameter.classification == goopal::api::required_positional)
               help_string += std::string("<") + parameter.name + std::string("> ");
             else if (parameter.classification == goopal::api::optional_named)
               help_string += std::string("{") + parameter.name + std::string("} ");
             else if (parameter.classification == goopal::api::required_positional_hidden)
               continue;
             else
               help_string += std::string("[") + parameter.name + std::string("] ");
           }
        ...
        void add_content_type_header(const fc::string& path, const fc::http::server::response& s ) {
              ...
              //     dlog( "CHECKING AUTH" );
               ...
               //    ilog( "\n\n\n  username: ${user}  password ${password}", ("path",r.path)("user",username)("password", password));
                ...
                //      elog( "\n\n\n Unauthorized ${path}, username: ${user}  password ${password}", ("path",r.path)("user",username)("password", password));
                      s.add_header( "WWW-Authenticate", "Basic realm=\"goopal wallet\"" );
         ...
         fc::http::reply::status_code handle_http_rpc(const fc::http::request& r, const fc::http::server::response& s )
         {
                ...
                try {
                   ...
                   if( fc::time_point(goopal::blockchain::now()) - _last_cache_clear_time  > fc::seconds(GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC) )
                   {
                      _last_cache_clear_time = goopal::blockchain::now();
                      _call_cache.clear();
                   }
         ...
         void encrypted_accept_loop(const goopal::blockchain::PrivateKeyType& server_key)
         {
        ...
        fc::variant dispatch_method_from_json_connection(fc::rpc::json_connection* con,
                                                         const goopal::api::MethodData& method_data,
                                                         const fc::variants& arguments)
        {
          // ilog( "arguments: ${params}", ("params",arguments) );
          if ((method_data.prerequisites & goopal::api::json_authenticated) &&
        ...
        fc::variant dispatch_authenticated_method(const goopal::api::MethodData& method_data,
                                                  const fc::variants& arguments_from_caller)
        {
          ...
          if (method_data.prerequisites & goopal::api::wallet_open)
            verify_wallet_is_open();
          if (method_data.prerequisites & goopal::api::wallet_unlocked)
            verify_wallet_is_unlocked();
          if (method_data.prerequisites & goopal::api::connected_to_network)
          ...
          {
            if (parameter.classification == goopal::api::required_positional
                || parameter.classification == goopal::api::required_positional_hidden)
            {
            ...
            else if (parameter.classification == goopal::api::optional_positional)
            {
    ...
    goopal::api::CommonApi* RpcServerImpl::get_client() const
    {
      return _client;
    }
    ...
    void RpcServerImpl::store_method_metadata(const goopal::api::MethodData& method_metadata)
    {
    ...
    std::string RpcServerImpl::help(const std::string& command_name) const
    {
      ...
      { //display detailed description of requested command
        ...
        if (alias_iter != _alias_map.end())
        {
          ...
          const goopal::api::MethodData& method_data = method_iter->second;
          help_string += "Usage:\n";
  ...
  RpcServer::RpcServer(goopal::client::Client* client) :
  ...
  const goopal::api::MethodData& RpcServer::get_method_data(const std::string& method_name)
  {
  ...
  std::vector<goopal::api::MethodData> RpcServer::get_all_method_data() const
  {
    std::vector<goopal::api::MethodData> result;
    for (const detail::RpcServerImpl::MethodMapType::value_type& value : my->_method_map)
      result.emplace_back(value.second);
    return result;
  }

  void RpcServer::register_method(goopal::api::MethodData data)
  {
