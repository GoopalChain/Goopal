...
//#include <goopal/mail/server.hpp>
...
const string GOP_MESSAGE_MAGIC = "Gop Signed Message:\n";
...
program_options::variables_map parse_option_variables(int argc, char** argv)
		{
         ...
         ("stop-before-block", program_options::value<uint32_t>(), "stop before given block number")
				...
				("growl-identifier", program_options::value<std::string>(), "A name displayed in growl messages to identify this gop_client instance")
				;
			...
			{
				std::cout << fc::json::to_pretty_string(goopal::client::version_info()) << "\n";
				exit(0);
			}
...
#ifdef GOP_TEST_NETWORK
						dir_name += "-Test" + std::to_string(GOP_TEST_NETWORK_VERSION);
#endif
						return dir_name;
					};

					datadir = fc::app_path() / get_os_specific_dir_name(GOP_BLOCKCHAIN_NAME);
				}
				return datadir;
      ...
      return cfg;
		...
		void load_checkpoints(const fc::path& data_dir)
		{
			try {
				...
				decltype(goopal::blockchain::detail::CHECKPOINT_BLOCKS) external_checkpoints;
				...
				{
				decltype(goopal::blockchain::detail::CHECKPOINT_BLOCKS) external_checkpoints;
				if (goopal::blockchain::detail::CHECKPOINT_BLOCKS.empty() || external_checkpoints.crbegin()->first >= goopal::blockchain::detail::CHECKPOINT_BLOCKS.crbegin()->first)
					{
						ulog("Using blockchain checkpoints from file: ${x}", ("x", checkpoint_file.preferred_string()));
						goopal::blockchain::detail::CHECKPOINT_BLOCKS = external_checkpoints;
						return;
					}
				}
				...
				if (!file_exception.valid())
				{
					...
					fc::json::save_to_file(goopal::blockchain::detail::CHECKPOINT_BLOCKS, checkpoint_file);
				}
				else
				{
				...
				if (!goopal::blockchain::detail::CHECKPOINT_BLOCKS.empty())
		...
		namespace detail
		{
				...
				const program_options::variables_map& option_variables)
			{
				if (option_variables.count("server") || option_variables.count("daemon") || cfg.rpc.enable)
				{
					...
					// goopal::rpc::rpc_server::config rpc_config(cfg.rpc);
...
void ClientImpl::start_delegate_loop()
{
    ...
    if (!_time_discontinuity_connection.connected())
        _time_discontinuity_connection = goopal::blockchain::time_discontinuity_signal.connect([=](){ reschedule_delegate_loop(); });
...
void ClientImpl::delegate_loop()
{
   ...
   {
      set_target_connections( GOP_NET_DELEGATE_DESIRED_CONNECTIONS );
   ...
   if( next_block_time.valid() )
   {
      ...
      if( *next_block_time <= now )
      {
         try
         {
            ...
            FC_ASSERT( (now - *next_block_time) < fc::seconds( GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC ),
      ...
      _delegate_loop_complete = fc::schedule( [=](){ delegate_loop(); }, scheduled_time, "delegate_loop" );
			...
			void ClientImpl::start_rebroadcast_pending_loop()
			{
				if (!_rebroadcast_pending_loop_done.valid() || _rebroadcast_pending_loop_done.ready())
					...
					fc::time_point::now() + fc::seconds((int64_t)(GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 1.3)),
					"rebroadcast_pending");
			}
			void ClientImpl::start_fork_update_time_loop()
			{
				if (!_fork_update_loop_done.valid() || _fork_update_loop_done.ready())
					...
					fc::time_point::now() + fc::seconds((int64_t)(GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 6)),
					"rebroadcast_pending");
			}
         ...
         if( fork_data && fork_data->is_known )
						{
							if (sync_mode && !fork_data->is_linked)
								FC_THROW_EXCEPTION(goopal::blockchain::unlinkable_block,
						...
						else
						{
							...
							if (sync_mode && !result.is_linked)
								FC_THROW_EXCEPTION(goopal::blockchain::unlinkable_block, "The blockchain accepted this block, but it isn't linked");
...
bool ClientImpl::on_new_transaction(const SignedTransaction& trx)
			...
			bool ClientImpl::has_item(const goopal::net::ItemId& id)
			{
			...
			*/
			bool ClientImpl::handle_message(const goopal::net::Message& message_to_handle, bool sync_mode)
			{
				try
				{
					...
					case block_message_type:
					{
						...
						goopal::blockchain::BlockIdType old_head_block = _chain_db->get_head_block_id();
				...
				catch (const goopal::blockchain::insufficient_relay_fee& original_exception)
				{
					FC_THROW_EXCEPTION(goopal::net::insufficient_relay_fee, "Insufficient relay fee; do not propagate!",
						("original_exception", original_exception.to_detail_string()));
				}
				catch (const goopal::blockchain::block_older_than_undo_history& original_exception)
				{
					FC_THROW_EXCEPTION(goopal::net::block_older_than_undo_history, "Block is older than undo history, stop fetching blocks!",
						("original_exception", original_exception.to_detail_string()));
				}
			}
			...
			*/
			std::vector<goopal::net::ItemHashType> ClientImpl::get_item_ids(uint32_t item_type,
				const std::vector<goopal::net::ItemHashType>& blockchain_synopsis,
				uint32_t& remaining_item_count,
				uint32_t limit /* = 2000 */)
			{
				// limit = 20; // for testing
				FC_ASSERT(item_type == goopal::client::block_message_type);
				...
				goopal::net::ItemHashType last_seen_block_hash;
				for (const goopal::net::ItemHashType& item_hash : boost::adaptors::reverse(blockchain_synopsis))
				{
					try
					{
				...
				std::vector<goopal::net::ItemHashType> hashes_to_return;
			...
			goopal::net::Message ClientImpl::get_item(const goopal::net::ItemId& id)
			{
				if (id.item_type == block_message_type)
				{
					//   uint32_t block_number = _chain_db->get_block_num(id.item_hash);
					goopal::client::BlockMessage block_message_to_send(_chain_db->get_block(id.item_hash));
			...
			uint32_t ClientImpl::get_block_number(const goopal::net::ItemHashType& block_id)
			{
				return _chain_db->get_block_num(block_id);
			}

			fc::time_point_sec ClientImpl::get_block_time(const goopal::net::ItemHashType& block_id)
			{
				if (block_id == goopal::net::ItemHashType())
				{
			...
			fc::time_point_sec ClientImpl::get_blockchain_now()
			{
				...
				return goopal::blockchain::nonblocking_now();
			}

			goopal::net::ItemHashType ClientImpl::get_head_block_id() const
			{
			...
			{
				return goopal::blockchain::estimate_last_known_fork_from_git_revision_timestamp(unix_timestamp);
			}
			...
			void ClientImpl::blocks_too_old_monitor_task()
			{
				...
				if (_chain_db->get_head_block().timestamp < goopal::blockchain::now() - fc::seconds(GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC * 2) &&
					!_sync_mode &&
				...
				if (!_blocks_too_old_monitor_done.canceled())
					...
					fc::time_point::now() + fc::seconds(GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC),
					"block_monitor_task");
			}
...
Client::Client(const std::string& user_agent,
               goopal::net::SimulatedNetworkPtr network_to_connect_to)
   : my( new detail::ClientImpl(this, user_agent) )
{
...
{ try {
    ...
    my->_wallet = std::make_shared<goopal::wallet::Wallet>( my->_chain_db, my->_config.wallet_enabled );
	my->_wallet->set_data_directory(wallet_dir / "wallets");

				/*  //delete?
				if (my->_config.mail_server_enabled)
				{
					my->_mail_server = std::make_shared<goopal::mail::server>();
					my->_mail_server->open(data_dir / "mail");
				}
				my->_mail_client = std::make_shared<goopal::mail::client>(my->_wallet, my->_chain_db);
    ...
    if( !my->_p2p_node )
       my->_p2p_node = std::make_shared<goopal::net::Node>( my->_user_agent );
	...
	if (!forks.empty())
	{
		...
		*/
		...
		goopal::rpc::RpcServerPtr Client::get_rpc_server()const { return my->_rpc_server; }
		goopal::net::NodePtr Client::get_node()const { return my->_p2p_node; }

		fc::variant_object version_info()
		{
   string client_version( goopal::utilities::git_revision_description );
   const size_t pos = client_version.find( '/' );
   if( pos != string::npos && client_version.size() > pos )
       client_version = client_version.substr( pos + 1 );
#ifdef GOP_TEST_NETWORK
			client_version += "-testnet-" + std::to_string(GOP_TEST_NETWORK_VERSION);
#endif

			fc::mutable_variant_object info;
			info["blockchain_name"] = GOP_BLOCKCHAIN_NAME;
			info["blockchain_description"] = GOP_BLOCKCHAIN_DESCRIPTION;
			info["client_version"] = client_version;
			info["gop_revision"] = goopal::utilities::git_revision_sha;
			info["gop_revision_age"] = fc::get_approximate_relative_time_string(fc::time_point_sec(goopal::utilities::git_revision_unix_timestamp));
...
#if defined(GOP_CLIENT_JENKINS_BUILD_NUMBER)
			info["jenkins_build_number"] = GOP_CLIENT_JENKINS_BUILD_NUMBER;
#endif
#if defined(GOP_CLIENT_JENKINS_BUILD_URL)
			info["jenkins_build_url"] = GOP_CLIENT_JENKINS_BUILD_URL;
#endif
...
void Client::start_networking(std::function<void()> network_started_callback)
{
   ...
   {
      goopal::net::ChainDownloader* chain_downloader = new goopal::net::ChainDownloader();
...
void Client::configure_from_command_line(int argc, char** argv)
{
   ...
   {
      my->_cli = new goopal::cli::Cli( this, nullptr, &std::cout );
      return;
   }
   // parse command-line options
   auto option_variables = parse_option_variables(argc,argv);

   fc::path datadir = goopal::client::get_data_dir(option_variables);
   fc::path walletDir = goopal::client::get_wallet_dir(option_variables, datadir);
   ...
   this->configure( datadir );
			...
			// else we use the default set in goopal::net::node
			...
			else //we will accept input from the console
			{
				...
				if (option_variables.count("log-commands") <= 0)
				{
					...
					my->_cli = new goopal::cli::Cli(this, my->_command_script_holder.get(), &std::cout);
				}
				else
				{
					...
					my->_cli = new goopal::cli::Cli(this, my->_command_script_holder.get(), my->_tee_stream.get());
   ...
   // start listening.  this just finds a port and binds it, it doesn't start
			...
			if (option_variables["upnp"].as<bool>())
			{
				...
				my->_upnp_service = std::unique_ptr<goopal::net::UpnpService>(new goopal::net::UpnpService);
			...
			if (option_variables.count("growl") || my->_config.growl_notify_endpoint)
			{
				...
				std::string gop_instance_identifier = "Gop";
				if (option_variables.count("growl-identifier"))
					gop_instance_identifier = option_variables["growl-identifier"].as<std::string>();
				else if (my->_config.growl_gop_client_identifier)
					gop_instance_identifier = *my->_config.growl_gop_client_identifier;
				my->_notifier = std::make_shared<GopGntpNotifier>(host_to_notify, port_to_notify, gop_instance_identifier, growl_password);
   ...
   {
      my->_chain_server = std::unique_ptr<goopal::net::ChainServer>(
               new goopal::net::ChainServer(my->_chain_db,
...
goopal::net::NodeIdType Client::get_node_id() const
{
...
void Client::set_daemon_mode(bool daemon_mode)
{
   init_cli();
		...
		{
			goopal::net::ItemId head_item_id;
			head_item_id.item_type = goopal::client::block_message_type;
			uint32_t last_block_num = my->_chain_db->get_head_block_num();
			if (last_block_num == (uint32_t)-1)
				head_item_id.item_hash = goopal::net::ItemHashType();
			else
				head_item_id.item_hash = my->_chain_db->get_head_block_id();
			my->_p2p_node->sync_from(head_item_id, goopal::blockchain::get_list_of_fork_block_numbers());
		...
		bool Client::handle_message(const goopal::net::Message& message, bool sync_mode)
		{
...
// 			}
		...
		goopal::api::CommonApi* Client::get_impl() const
		{
			return my.get();
		}
