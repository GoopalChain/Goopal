...
void detail::ClientImpl::wallet_create(const string& wallet_name, const string& password, const string& brain_key)
{
  ...
  if( brain_key.size() && brain_key.size() < GOP_WALLET_MIN_BRAINKEY_LENGTH ) FC_CAPTURE_AND_THROW( brain_key_too_short );
  if( password.size() < GOP_WALLET_MIN_PASSWORD_LENGTH ) FC_CAPTURE_AND_THROW( password_too_short );
...
void read_keys( const fc::variant& vo, vector<PrivateKeyType>& keys, const string& password )
{
    ...
    try
    {
      ...
      const auto key = goopal::utilities::wif_to_key( wif_key );
    ...
    catch( const goopal::wallet::invalid_password& e )
    {
        throw;
    }
    catch( ... )
  ...
  if( _config.wallet_callback_url.size() > 0 )
  {
     _http_callback_signal_connection =
        _wallet->wallet_claimed_transaction.connect(
            [=]( LedgerEntry e ) { this->wallet_http_callback( _config.wallet_callback_url, e ); } );
  }
}
...
vector<goopal::wallet::PrettyTransaction> detail::ClientImpl::wallet_transaction_history_splite(const std::string& account_name,
																							   const std::string& asset_symbol, 
																							   int32_t limit, 
																							   int32_t transaction_type)const
{
	try {
		const auto history = _wallet->get_pretty_transaction_history(account_name, 0, -1, asset_symbol, (goopal::wallet::Wallet::TrxType)transaction_type,true);
...
{
	int first = address.find_first_of("GOP");
...
// vector<goopal::wallet::escrow_summary> ClientImpl::wallet_escrow_summary( const string& account_name ) const
// { try {
