...
{
   if( first > 0 ) --first;
   FC_ASSERT( first < GOP_BLOCKCHAIN_NUM_DELEGATES ,"Only the first 99 delegates are active");
   FC_ASSERT(first + count <= GOP_BLOCKCHAIN_NUM_DELEGATES,  "Only the first 99 delegates are active");
   vector<AccountIdType> all_delegate_ids = _chain_db->get_active_delegates();
   FC_ASSERT( all_delegate_ids.size() == GOP_BLOCKCHAIN_NUM_DELEGATES ,"The num of active delegates should equal to GOP_BLOCKCHAIN_NUM_DELEGATES");
...
vector<string> ClientImpl::blockchain_list_missing_block_delegates( uint32_t block_num )
{
   ...
   timestamp += GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
   while (timestamp != this_block.timestamp)
   // if time interval between prev_block and this block greater than GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC,
...
oAccountEntry detail::ClientImpl::blockchain_get_account( const string& account )const
{
   try
   {
      ...
      else if( account.substr( 0, string( GOP_ADDRESS_PREFIX ).size() ) == GOP_ADDRESS_PREFIX ) {
...
variant_object ClientImpl::blockchain_get_info()const
{
   ...
   info["name"]                                 = GOP_BLOCKCHAIN_NAME;
   info["symbol"]                               = GOP_BLOCKCHAIN_SYMBOL;
   info["address_prefix"]                       = GOP_ADDRESS_PREFIX;
   info["db_version"]                           = GOP_BLOCKCHAIN_DATABASE_VERSION;
   info["genesis_timestamp"]                    = _chain_db->get_genesis_timestamp();

   info["block_interval"]                       = GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC;
   info["delegate_num"]                         = GOP_BLOCKCHAIN_NUM_DELEGATES;
   ...
   info["name_size_max"]                        = GOP_BLOCKCHAIN_MAX_NAME_SIZE;
   info["memo_size_max"]                        = GOP_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE;
   info["data_size_max"]                        = GOP_BLOCKCHAIN_MAX_NAME_DATA_SIZE;

   info["symbol_size_max"]                      = GOP_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE;
   info["symbol_size_min"]                      = GOP_BLOCKCHAIN_MIN_SYMBOL_SIZE;
   info["asset_shares_max"]                     = GOP_BLOCKCHAIN_MAX_SHARES;
   info["short_symbol_asset_reg_fee"]           = _chain_db->get_asset_registration_fee( GOP_BLOCKCHAIN_MIN_SYMBOL_SIZE );
   info["long_symbol_asset_reg_fee"]            = _chain_db->get_asset_registration_fee( GOP_BLOCKCHAIN_MAX_SUB_SYMBOL_SIZE );

   info["statistics_enabled"]                   = _chain_db->get_statistics_enabled();

   info["relay_fee"]                            = _chain_db->get_relay_fee();
   info["max_pending_queue_size"]               = GOP_BLOCKCHAIN_MAX_PENDING_QUEUE_SIZE;
   info["max_trx_per_second"]                   = GOP_BLOCKCHAIN_MAX_TRX_PER_SECOND;
	...
	if (fp != NULL)
	{
		while (true)
		{
			...
			if (fgets(c, 256, fp) != NULL)
			{
				...
				std::string allstring = (std::string)goopal::blockchain::Address(goopal::blockchain::PtsAddress(pow_address));
				allstring += balance;
...
goopal::blockchain::BlockchainSecurityState ClientImpl::blockchain_get_security_state()const
{
   ...
   state.estimated_confirmation_seconds = (uint32_t)(required_confirmations * GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
   state.participation_rate = participation_rate;
   if (!blockchain_is_synced())
   {
      state.alert_level = goopal::blockchain::BlockchainSecurityState::grey;
   }
   else if (required_confirmations <= GOP_BLOCKCHAIN_NUM_DELEGATES / 2
            && participation_rate > 80)
   {
      state.alert_level = goopal::blockchain::BlockchainSecurityState::green;
   }
   else if (required_confirmations > GOP_BLOCKCHAIN_NUM_DELEGATES
            || participation_rate < 60)
   {
      state.alert_level = goopal::blockchain::BlockchainSecurityState::red;
   }
   else
   {
      state.alert_level = goopal::blockchain::BlockchainSecurityState::yellow;
   }
...
{
   return (blockchain::now() - _chain_db->get_head_block().timestamp) < fc::seconds(GOP_BLOCKCHAIN_NUM_DELEGATES * GOP_BLOCKCHAIN_BLOCK_INTERVAL_SEC);
}
...
bool ClientImpl::blockchain_verify_signature(const string& signer, const fc::sha256& hash, const fc::ecc::compact_signature& signature) const
{
    ...
    catch (...)
    {
        try {
           ...
           // NB LHS of == operator is goopal::blockchain::public_key_type and RHS is fc::ecc::public_key,
...
goopal::blockchain::Asset ClientImpl::blockchain_unclaimed_genesis() const
{
