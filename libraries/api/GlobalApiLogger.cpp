...
namespace detail
{
    ...
    public:
        ...
        virtual uint64_t log_call_started( const goopal::api::CommonApi* target, const fc::string& name, const fc::variants& args ) override;
    ...
    uint64_t GlobalApiLoggerImpl::log_call_started( const goopal::api::CommonApi* target, const fc::string& name, const fc::variants& args )
    {
    ...
    void GlobalApiLoggerImpl::log_call_finished( uint64_t call_id, const goopal::api::CommonApi* target, const fc::string& name, const fc::variants& args, const fc::variant& result )
    {
        if( name == "debug_get_client_name" )
            return;
        for( ApiLogger* logger : this->active_loggers )
