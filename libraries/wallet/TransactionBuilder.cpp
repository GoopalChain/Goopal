...
void  TransactionBuilder::set_wallet_implementation(std::unique_ptr<goopal::wallet::detail::WalletImpl>& wimpl)
{
    _wimpl = wimpl.get();
}
...
TransactionBuilder& TransactionBuilder::deposit_asset(const goopal::wallet::WalletAccountEntry& payer,
                                                        const goopal::blockchain::AccountEntry& recipient,
														const Asset& amount,
                                                        const string& memo,									
...
{ try {
   ...
   if( memo.size() > GOP_BLOCKCHAIN_MAX_EXTENDED_MEMO_SIZE )
...
TransactionBuilder& TransactionBuilder::deposit_asset_with_escrow(const goopal::wallet::WalletAccountEntry& payer,
                                                        const goopal::blockchain::AccountEntry& recipient,
                                                        const goopal::blockchain::AccountEntry& escrow_agent,
...
{ try {
   ...
   // Don't automatically truncate memos as long as users still depend on them via deposit ops rather than mail
   if( memo.size() > GOP_BLOCKCHAIN_MAX_MEMO_SIZE )
       FC_CAPTURE_AND_THROW( memo_too_long, (memo) );
...
{ try {
    ...
    if( market_fee >= 0 || market_fee <= GOP_BLOCKCHAIN_MAX_UIA_MARKET_FEE )
       actual_market_fee = GOP_BLOCKCHAIN_MAX_UIA_MARKET_FEE * market_fee;
