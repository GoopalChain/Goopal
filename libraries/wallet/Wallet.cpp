...
namespace detail {
   ...
   void WalletImpl::upgrade_version()
   {
       ...
       if( current_version > GOP_WALLET_VERSION )
       {
           FC_THROW_EXCEPTION( unsupported_version, "Wallet version newer than client supports!",
                               ("wallet_version",current_version)("supported_version",GOP_WALLET_VERSION) );
       }
       else if( current_version == GOP_WALLET_VERSION )
       {
           return;
       }
   ...
   { try {
      ...
      if( password.size() < GOP_WALLET_MIN_PASSWORD_LENGTH )
    ...
    *  returned will contain no more than GOP_BLOCKCHAIN_MAX_SLATE_SIZE delegates.
    */
   SlateEntry WalletImpl::get_delegate_slate( const VoteStrategy strategy )const
   {
      ...
      {
          slate_size = std::min<size_t>( GOP_BLOCKCHAIN_MAX_SLATE_SIZE, for_candidates.size() );
      }
      else if( strategy == vote_random )
      {
          slate_size = std::min<size_t>( GOP_BLOCKCHAIN_MAX_SLATE_SIZE / 3, for_candidates.size() );
          slate_size = rand() % ( slate_size + 1 );
      }
      else if( strategy == vote_recommended && for_candidates.size() < GOP_BLOCKCHAIN_MAX_SLATE_SIZE )
      {
          ...
          while( for_candidates.size() < GOP_BLOCKCHAIN_MAX_SLATE_SIZE && recommended_candidate_ranks.size() > 0 )
          {
            int best_rank = 0;
      ...
      FC_ASSERT( entry.slate.size() <= GOP_BLOCKCHAIN_MAX_SLATE_SIZE );
   ...
   { try {
      ...
      try
      {
          ...
          open( wallet_name );
          unlock( password, GOP_WALLET_DEFAULT_UNLOCK_TIME_SEC );
      }
      catch( ... )
      {
   ...
   { try {
      ...
      try
      {
          ...
          close();
          open( wallet_name );
          unlock( passphrase, GOP_WALLET_DEFAULT_UNLOCK_TIME_SEC );
      }
      catch( ... )
      {
   ...
   { try {
	  ...
	  if (passphrase.size() < GOP_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
   ...
   void Wallet::change_passphrase(const string & old_passphrase, const string& new_passphrase)
   {
	   try {
		   if (NOT is_open()) FC_CAPTURE_AND_THROW(wallet_closed);
		   if (NOT is_unlocked()) FC_CAPTURE_AND_THROW(wallet_locked);
		   if (new_passphrase.size() < GOP_WALLET_MIN_PASSWORD_LENGTH) FC_CAPTURE_AND_THROW(password_too_short);
   ...
   { try {
      ...
      auto key = goopal::utilities::wif_to_key( wif_key );
      if( key.valid() )
      {
   ...
   VoteSummary Wallet::get_vote_status(const string& account_name)
   {
       ...
       for( const auto& item : items )
       {
           ...
           for( const auto& entry : entrys )
           {
               if( entry.asset_id() != 0 )
                   continue;
               if( entry.slate_id() != my_slate.id() && entry.balance > 1 * GOP_BLOCKCHAIN_PRECISION ) // ignore dust
               ...
               total_possible += entry.get_spendable_balance( my->_blockchain->now() ).amount * GOP_BLOCKCHAIN_MAX_SLATE_SIZE;
           }
       }
   ...
   { try {
      ...
      const auto version = goopal::client::version_info()["client_version"].as_string();
   ...
   { try {
       ...
       for( const auto& item : items )
       {
           ...
           if( name.find( GOP_ADDRESS_PREFIX ) != 0 )
               continue;
   ...
   { try {
	   ...
	   trx.deposit(receiver_account.active_address(), Asset(amount_to_withdraw, 0));
...
//                                );
       ...
       trans_entry.ledger_entries.push_back(entry);
       trans_entry.fee = required_fees;
       trans_entry.extra_addresses.push_back(receiver_account.active_address());
       trans_entry.trx = trx;
	   //changed by CCW for deleting Titan Transfer
...
//      required_signatures.insert( issuer_account->active_key() );
  ...
  //    if( sign )
   ...
   { try {
      ...
      Asset xts_fee( GOP_WALLET_DEFAULT_TRANSACTION_FEE, 0 );
      try
      {
   ...
   { try {
       ...
       if( secs > GOP_BLOCKCHAIN_MAX_TRANSACTION_EXPIRATION_SEC )
