...
namespace goopal { namespace wallet {
   ...
   namespace detail {
     class WalletDbImpl
     {
        public:
           ...
           goopal::db::LevelMap<int32_t,GenericWalletEntry> _entrys;
           ...
           { try {
               ...
               // Cache address map
               self->btc_to_gop_address[ key_address ] = key_address;
               self->btc_to_gop_address[ Address( PtsAddress( key_entry.public_key, false, 0  ) ) ] = key_address; // Uncompressed BTC
               self->btc_to_gop_address[ Address( PtsAddress( key_entry.public_key, true,  0  ) ) ] = key_address; // Compressed BTC
               self->btc_to_gop_address[ Address( PtsAddress( key_entry.public_key, false, 56 ) ) ] = key_address; // Uncompressed PTS
               self->btc_to_gop_address[ Address( PtsAddress( key_entry.public_key, true,  56 ) ) ] = key_address; // Compressed PTS
		   ...
		   void remove_generic_wallet_entry(GenericWalletEntry& rec)
		   {
			   //从db中将钱包记录删除
			   try{
   ...
   { try {
       FC_ASSERT( is_open() ,"Wallet not open!");
       const auto address_map_iter = btc_to_gop_address.find( derived_address );
       if( address_map_iter != btc_to_gop_address.end() )
       {
